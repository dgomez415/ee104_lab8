...
def draw():
    global game_over, score, say_dance, score1, score2, dance_length
    ...
    if not game_over:
        ...
        screen.draw.text("Level: " +
                         str(dance_length - 3), color="black",
                         topleft=(10, 40))
        if say_dance:
...
def update():
    global game_over, current_move, moves_complete, rounds, dance_length, score1, score2
    if not game_over:
        if moves_complete:
            if(rounds %2 == 0):
               score2 += 1
